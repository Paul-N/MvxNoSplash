...
using PreserveAttribute = MvvmCross.PreserveAttribute;
...
#nullable enable
    ...
    /// </summary>
    [Register("org.nosplash._MvxLayoutInflater")]
    public class _MvxLayoutInflater : LayoutInflater
    {
        ...
        public _MvxLayoutInflater(Context context)
            : base(context)
        {
        ...
        public _MvxLayoutInflater(LayoutInflater original, Context? newContext, MvxBindingVisitor? bindingVisitor, bool cloned)
        ...
        public _MvxLayoutInflater(IntPtr handle, JniHandleOwnership transfer)
        ...
        {
            return new _MvxLayoutInflater(this, newContext, _bindingVisitor, true);
        }
        ...
        {
            if (Debug)
                MvxLogHost.GetLog<_MvxLayoutInflater>()?.Log(LogLevel.Trace, "{Tag} - ... OnCreateView 3 ... {name}", Tag, name);
        ...
        {
            if (Debug)
                MvxLogHost.GetLog<_MvxLayoutInflater>()?.Log(LogLevel.Trace, "{Tag} - ... OnCreateView 2 ... {name}", Tag, name);
        ...
        {
            if (Debug)
                MvxLogHost.GetLog<_MvxLayoutInflater>()?.Log(LogLevel.Trace, "{Tag} - ... OnCreateView 4 ... {name}", Tag, name);
        ...
        {
            if (Debug)
                MvxLogHost.GetLog<_MvxLayoutInflater>()?.Log(LogLevel.Trace, "{Tag} - ... PhoneLayoutInflaterOnCreateView ... {name}", Tag, name);
        ...
        public void setFactory(IFactory factory)
        {
            ...
            if (!(factory is _MvxLayoutInflaterCompat.FactoryWrapper))
            {
                Factory = new _MvxLayoutInflaterCompat.FactoryWrapper(new DelegateFactory1(factory, _bindingVisitor));
                return;
            }
        ...
        {
            // Wrap the incoming factory if we need to.
            if (!(factory2 is _MvxLayoutInflaterCompat.FactoryWrapper2))
            {
                Factory2 = new _MvxLayoutInflaterCompat.FactoryWrapper2(new DelegateFactory2(factory2, _bindingVisitor));
                return;
            }
        ...
        private void SetupLayoutFactories(bool cloned)
        {
            ...
            if (Factory2 != null && !(Factory2 is _MvxLayoutInflaterCompat.FactoryWrapper2))
            {
        ...
        private void SetPrivateFactoryInternal()
        {
            ...
            {
                MvxLogHost.GetLog<_MvxLayoutInflater>()?.Log(LogLevel.Warning, ex, "Cannot invoke LayoutInflater.setPrivateFactory :\n{0}", ex.StackTrace);
            }
        ...
        {
            if (Debug)
                MvxLogHost.GetLog<_MvxLayoutInflater>()?.Log(LogLevel.Trace, "{Tag} - ... CreateCustomViewInternal ... {name}", Tag, name);
        ...
        protected IMvxAndroidViewFactory? AndroidViewFactory
        {
            get
            {
                ...
                if (Mvx.IoCProvider == null)
                {
                    ...
                    MvxLogHost.GetLog<_MvxLayoutInflater>()?.Log(LogLevel.Trace, "{Tag} - ... AndroidViewFactory IoCProvider is null!", Tag);
                    return null;
                }
        ...
        protected IMvxLayoutInflaterHolderFactoryFactory? FactoryFactory
        {
            get
            {
                ...
                {
                    // if IoCProvider is null, Log instance will probably be null too
                    MvxLogHost.GetLog<_MvxLayoutInflater>()?.Log(LogLevel.Error, "{Tag} - ... FactoryFactory IoCProvider is null!", Tag);
                    return null;
                }
        ...
        private class DelegateFactory2 : IMvxLayoutInflaterFactory
        {
            ...
            {
                if (Debug)
                    MvxLogHost.GetLog<_MvxLayoutInflater>()?.Log(LogLevel.Trace, "{Tag} - ... OnCreateView ... {name}", DelegateFactory2Tag, name);
        ...
        private class DelegateFactory1 : IMvxLayoutInflaterFactory
        {
            ...
            {
                if (Debug)
                    MvxLogHost.GetLog<_MvxLayoutInflater>()?.Log(LogLevel.Trace, "{Tag} - ... OnCreateView ... {name}", DelegateFactory1Tag, name);
        ...
        private class PrivateFactoryWrapper2 : Object, IFactory2
        {
            ...
            private readonly MvxBindingVisitor _bindingVisitor;
            private readonly _MvxLayoutInflater _inflater;

            internal PrivateFactoryWrapper2(IFactory2 factory2, _MvxLayoutInflater inflater,
...
#pragma warning restore 8618
            ...
            {
                if (Debug)
                    MvxLogHost.GetLog<_MvxLayoutInflater>()?.Log(LogLevel.Trace, "{Tag} - ... OnCreateView 2 ... {name}", PrivateFactoryWrapper2Tag, name);
            ...
            {
                if (Debug)
                    MvxLogHost.GetLog<_MvxLayoutInflater>()?.Log(LogLevel.Trace, "{Tag} - ... OnCreateView 3 ... {name}", PrivateFactoryWrapper2Tag, name);
